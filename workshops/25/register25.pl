...
#record it unless debug
  if ($name eq "debug") {
    ...
    unless (open REGISTERLOG, ">>registerlog25") {
      print $query->start_html("Registration Failed");
      print "<h1>Registration Failed!</h1>\n";
...
#mail me
    `echo "Someone registered for the Soar workshop 25!" | sendmail voigtjr\@umich.edu`;
...
#display success and verification message if success
  ...
  if ($tutorial eq "beginner") {
    print "<p>You are going to attend the beginner tutorials on June 13-14.</p>\n";
  }
  if ($tutorial eq "advanced") {
    print "<p>You are going to attend the advanced tutorial on June 14.</p>\n";
  }
  if ($tutorial eq "none") {
  ...
  print "<hr>\n<a href=\"http://winter.eecs.umich.edu/workshop25\">Soar Workshop 25 home</a><br>";
